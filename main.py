...
def get_stale_branches(repo, tw_years=TW_YEARS):
    ...
    for branch in repo.get_branches():
        commit_date = branch.commit.commit.committer.date  # Timezone-aware datetime
        commit_date = commit_date.replace(tzinfo=None)  # Convert to naive UTC datetime
...
def display_summary(repo_summaries):
    ...
    for repo_name, summary in repo_summaries.items():
        ...
        if summary["stale_branches"] == summary["total_branches"]:
            print("   - Recommendation: Consider deleting this repository as all branches are stale.")
